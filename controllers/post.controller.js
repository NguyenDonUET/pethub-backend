...
const createPost = async (req, res, next) => {
  ...
  try {
    ...
    if (user.role === "buyer" || (user.role === "seller" && !user.isApproved)) {
      res
        .status(403)
        .send({ message: "You are not allowed to create new post." });
      return;
    }
  } catch (err) {
    console.log(err);
    res
      .status(500)
      .send({ message: "Authorization failed, please try again later." });
    return;
  }
  ...
  try {
    ...
    res
      .status(500)
      .send({ message: "Database Register failed, please try again later." });
    return;
  }
...
const getPosts = async (req, res, next) => {
  ...
  try {
    posts = await Post.find({
      ...
      .sort(order + orderBy)
      .populate("creator", "username email avatar id");
    if (orderBy === "star") {
      posts.sort((a, b) => {
        if (order === "-") {
    ...
    res.status(404).send("Error get posts!!!");
    console.log(err);
    return;
  }
...
const getPostById = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(500)
      .send({ message: "Something went wrong, could not find a post." });
    console.log(err);
    return;
  }
  ...
  if (!post) {
    res
      .status(404)
      .send({ message: "Could not find post for the provided id." });
    return;
  }
...
const postReview = async (req, res, next) => {
  ...
  try {
    ...
    let existingReview = post.reviews.filter(
      (value) => value.creator == review.creator
    );
